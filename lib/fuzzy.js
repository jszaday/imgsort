...
/**
 * The greedy first-match subsequence positions of `query` within `str`
 * (case-insensitive; indices are into the original `str`). Mirrors the walk
 * `fuzzyScore` does internally. Returns `null` for an empty query or no match.
 * @param {string} query
 * @param {string} str
 * @returns {number[] | null}
 */
export function fuzzyMatchIndices(query, str) {
