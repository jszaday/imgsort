 ...
 *   | { t: 'create', name: string, image: string })
 *   & { disabled?: boolean }} Txn
 * `disabled: true` keeps the entry in the log (and the session file) but makes
 * {@link replay} skip it — the Actions tab toggles this.
 */
 ...
 * @param {Txn[]} txns
 * @param {{ clearUncategorized?: boolean }} [opts] - forwarded to applyToggle
 * @returns {{ folders: string[], assignments: Map<string, Set<string>> }}
 */
export function replay(base, txns, opts = {}) {
    const images = base.images || [];
    const discovered = base.discovered || {};
    const toggleOpts = { clearUncategorized: opts.clearUncategorized !== false };
    ...
    for (const tx of txns || []) {
        if (tx.disabled === true) continue; // Actions tab: kept in the log, not applied
        if (tx.t === 'create') {
            ...
            if (folders.includes(tx.name)) {
                assignments.set(
                    tx.image,
                    applyToggle(assignments.get(tx.image) || [], tx.name, toggleOpts)
                );
            }
        } else if (tx.t === 'toggle') {
            if (present.has(tx.image) && folders.includes(tx.category)) {
                assignments.set(
                    tx.image,
                    applyToggle(assignments.get(tx.image) || [], tx.category, toggleOpts)
                );
            }
        } else if (tx.t === 'set') {
